...
3. **扫描与过滤**：自主扫描根目录（/Users/dr4/WorkSpace/context-infrastructure）下的变动。
4. **写入记忆**：将针对 {target_date} 的 🔴 🟡 🟢 观测结果直接写入或追加到 `/Users/dr4/WorkSpace/context-infrastructure/contexts/memory/OBSERVATIONS.md`。**鼓励使用命令行 append**（如 `echo "..." >> OBSERVATIONS.md` 或 `tee -a`），避免对大文件做全文编辑。
