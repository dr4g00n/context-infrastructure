...
`rules/` 下的文件是活文档，由你主动演进，不被动等待指令。

**触发条件**（满足任一即启动检视）：
1. 用户指出"你违背了 SOUL.md"或任何行为规范
2. 同一类错误发生 2 次以上
3. 用户说"这是原则问题"或类似表述（含对进化循环自身原则的质疑）——**语义触发，由 AI 在会话中主动识别，不由 hook 硬拦截**
4. 发现自我定义与实际行为不一致

**触发层级说明**：
- **硬触发**（条件 1、2 的前半部分）：由 Stop Hook 通过正则自动检测，直接 block AI 并进入提案流程。
- **软触发 / 语义触发**（条件 2 的后半部分、条件 3、条件 4）：需要 AI 主动分析会话语义、查阅历史记忆后自行判断。这类信号不适合用简单正则匹配，避免误拦截正常讨论。
...
**行动流程**：
1. 完成当前任务（不中断工作流）
2. 定位偏差：规则缺漏？表述不够强？缺乏执行锚点？
3. 提出修改：哪个文件、改什么、为什么、预期效果
4. 用户确认后执行

**反惰性原则**：判断"不需要修改"时必须给出理由。规则已覆盖但行为仍偏离，问题出在表述、位置、锚点三选一，然后修。
